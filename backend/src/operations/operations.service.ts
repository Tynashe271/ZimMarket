...
@Injectable() export class OperationsService {
  ...
  async storefront(userId:string,accountType:string,slug: string) {
    ...
    // Review author names are public storefront content shown to any shopper,
    // same as the review text and star rating -- not a per-viewer authorization
    // case, so this reads the reviewer's name via the system context.
    const [rating,reviews]=await Promise.all([this.prisma.review.aggregate({where:{businessId:business.id,hidden:false},_avg:{rating:true},_count:true}),this.prisma.withSystemContext(tx=>tx.review.findMany({where:{businessId:business.id,hidden:false},select:{id:true,rating:true,comment:true,createdAt:true,customer:{select:{fullName:true}}},orderBy:{createdAt:'desc'},take:20}))]);
