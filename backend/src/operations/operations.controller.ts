...
import { JwtAuthGuard } from '../auth/jwt-auth.guard'; import { AuthUser } from '../auth/jwt.strategy'; import { CurrentUser } from '../common/current-user.decorator'; import { PaginationQueryDto } from '../common/pagination.dto'; import { OperationsService } from './operations.service';
...
class DiscoverQueryDto extends PaginationQueryDto { @IsOptional() @IsString() city?: string; @IsOptional() @IsString() province?: string; @IsOptional() @IsString() tag?: string; }
@Controller() export class OperationsController {
  constructor(private readonly s: OperationsService) {}
  @Get('public/businesses') @UseInterceptors(CacheInterceptor) @CacheTTL(30_000) discover(@Query() query: DiscoverQueryDto) { return this.s.discover(query.city, query.province, query.tag, query); }
