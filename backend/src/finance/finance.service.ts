...
@Injectable() export class FinanceService {
  ...
  async initiatePayment(userId: string, accountType: string, orderId: string, provider: string, idempotencyKey: string) {
    ...
    const order = await this.prisma.withContext({ userId, accountType }, tx => tx.order.findFirst({ where: { id: orderId, customerId: userId }, include: { business: { include: { fiscalisation: true } }, customer: { select: { email: true } } } }));
    ...
    const payment=await this.prisma.withContext({ userId, accountType }, tx => tx.payment.upsert({ where: { idempotencyKey }, update: {}, create: { orderId, provider, idempotencyKey, amount: order.total, currency: order.currency } }));
    ...
    if (mode==='paynow' && online && payment.status==='PENDING') {
      ...
      await this.prisma.withContext({ userId, accountType }, tx => tx.payment.update({ where: { id: payment.id }, data: { providerCheckoutRef: result.pollUrl } }));
  ...
  async receipt(userId:string,accountType:string,paymentId:string){
    const payment=await this.prisma.withContext({userId,accountType},tx=>tx.payment.findFirst({
      where:{id:paymentId,...(accountType===AccountType.ADMIN?{}:accountType===AccountType.BUSINESS?{order:{business:{members:{some:{userId}}}}}:{order:{customerId:userId}})},
      include:{order:{include:{business:{select:{id:true,name:true}},customer:{select:{id:true,fullName:true,email:true,phone:true}},items:true,recipient:true,invoice:true}}},
    }));
    if(!payment)throw new NotFoundException('Payment not found');
    if(payment.status!=='SUCCEEDED')throw new BadRequestException('Receipt is available after payment confirmation');
    return{receiptNumber:`RCT-${payment.id.slice(0,8).toUpperCase()}`,invoiceNumber:payment.order.invoice?.number,paidAt:payment.updatedAt,provider:payment.provider,providerReference:payment.providerRef,amount:payment.amount,currency:payment.currency,order:{id:payment.order.id,status:payment.order.status,items:payment.order.items,recipient:payment.order.recipient},customer:payment.order.customer,business:payment.order.business};
  }
  ...
  async pollPaymentStatus(userId: string, paymentId: string) {
    const payment = await this.prisma.withContext({ userId }, tx => tx.payment.findFirst({ where: { id: paymentId, order: { customerId: userId } } }));
    if (!payment) throw new NotFoundException('Payment not found');
  ...
  private async finalizePayment(idempotencyKey: string, providerRef: string, status: 'SUCCEEDED'|'FAILED') {
    const payment = await this.prisma.withSystemContext(tx => tx.payment.findUnique({ where: { idempotencyKey }, include: { order: {include:{invoice:true,customer:true,items:true,recipient:true,business:true}} } }));
    if (!payment) throw new NotFoundException('Payment not found');
