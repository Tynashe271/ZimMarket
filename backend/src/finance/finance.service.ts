...
@Injectable() export class FinanceService {
  ...
  async initiatePayment(userId: string, accountType: string, orderId: string, provider: string, idempotencyKey: string) {
    ...
    const order = await this.prisma.order.findFirst({ where: { id: orderId, customerId: userId }, include: { business: { include: { fiscalisation: true } } } });
    ...
    return {...payment,mode:this.config.get('PAYMENT_PROVIDER','development'),online:onlineMethods.includes(provider),checkoutUrl:null,requiresDemoConfirmation:this.config.get('PAYMENT_PROVIDER','development')==='development'&&onlineMethods.includes(provider),instructions:cashMethods.includes(provider)?(provider==='CASH_ON_DELIVERY'?'Pay the authorized courier when your order arrives.':'Pay at the shop when collecting your order.'):'Continue with the selected secure payment provider.'};
  }
  ...
  async webhook(signature: string, payload: { idempotencyKey: string; providerRef: string; status: 'SUCCEEDED'|'FAILED' }) {
    ...
    const payment = await this.prisma.payment.findUnique({ where: { idempotencyKey: payload.idempotencyKey }, include: { order: {include:{invoice:true,customer:true,items:true,recipient:true,business:true}} } });
