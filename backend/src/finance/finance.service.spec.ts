...
// against the same fake `tx`-shaped object so existing assertions still work.
function makePrisma(tx:Record<string,unknown>={}){return{...tx,withContext:jest.fn((_context:unknown,fn:(tx:unknown)=>unknown)=>fn(tx)),withSystemContext:jest.fn((fn:(tx:unknown)=>unknown)=>fn(tx))}}
describe('Financial webhooks',()=>{it('rejects an invalid refund signature before database access',async()=>{const findFirst=jest.fn();const prisma=makePrisma({refundRequest:{findFirst}});const config={getOrThrow:jest.fn().mockReturnValue('secret')};const service=new FinanceService(prisma as never,config as never,{} as never);await expect(service.completeRefund('invalid',{refundId:'00000000-0000-4000-8000-000000000000',providerRef:'ref'})).rejects.toBeInstanceOf(UnauthorizedException);expect(findFirst).not.toHaveBeenCalled()})});
describe('Paynow payment initiation',()=>{
  function setup(paynowResult:unknown){
    const order=validOrder();
    const update=jest.fn().mockResolvedValue({});
    const prisma=makePrisma({order:{findFirst:jest.fn().mockResolvedValue(order)},payment:{upsert:jest.fn().mockResolvedValue({id:'p1',status:'PENDING'}),update}});
    ...
    return{service:new FinanceService(prisma as never,config as never,paynow as never),update,paynow};
  }
  it('returns a real checkout url when Paynow accepts the transaction',async()=>{
    const{service,update}=setup({success:true,browserUrl:'https://paynow.co.zw/pay/abc',pollUrl:'https://paynow.co.zw/poll/abc'});
    ...
    expect(update).toHaveBeenCalledWith({where:{id:'p1'},data:{providerCheckoutRef:'https://paynow.co.zw/poll/abc'}});
  });
...
describe('Paynow result callback',()=>{
  it('finalizes the payment when Paynow confirms it was paid',async()=>{
    ...
    const tx={payment:{findUnique:jest.fn().mockResolvedValue(payment),update:jest.fn().mockResolvedValue({...payment,status:'SUCCEEDED'})},order:{update:jest.fn()},invoice:{update:jest.fn()},notificationOutbox:{create:jest.fn()},ledgerEntry:{createMany:jest.fn()}};
    const prisma={withSystemContext:jest.fn((fn:(tx:unknown)=>unknown)=>fn(tx))};
  ...
  it('rejects a callback with an invalid signature before touching the database',async()=>{
    ...
    const prisma=makePrisma({payment:{findUnique}});
