import { ForbiddenException, Injectable, NotFoundException } from '@nestjs/common'; import { AccountType, BusinessRole, Prisma, QuoteStatus } from '@prisma/client'; import { JobsService } from '../jobs/jobs.service'; import { PrismaService } from '../prisma/prisma.service';
@Injectable() export class OpportunitiesService {
  ...
  mine(userId: string) { return this.prisma.customerRequest.findMany({ where: { customerId: userId }, include: { quotes: { include: { business: { select: { name: true, slug: true, verificationLevel: true } }, items: true } } }, orderBy: { createdAt: 'desc' } }); }
  async relevant(userId: string, businessId: string) {
