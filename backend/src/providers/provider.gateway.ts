import{BadRequestException,Injectable,NotFoundException,UnauthorizedException}from'@nestjs/common';import{ConfigService}from'@nestjs/config';import{Prisma}from'@prisma/client';import{Storage}from'@google-cloud/storage';import{createHash,createHmac,timingSafeEqual}from'crypto';import{mkdir,readFile,writeFile}from'fs/promises';import{dirname,extname,isAbsolute,resolve,sep}from'path';import{PrismaService}from'../prisma/prisma.service';
// sharp@0.35's CJS build (dist/index.cjs) does a bare `module.exports = Sharp`
// with no `.default`, so a plain default import fails at runtime without
// esModuleInterop (see src/types/sharp.d.ts for the matching type shim).
// eslint-disable-next-line @typescript-eslint/no-require-imports
import sharp = require('sharp');import{AfricasTalkingProvider}from'./africastalking.provider';import{TwilioProvider}from'./twilio.provider';import{SmsProvider,SmsSendResult}from'./sms-provider.interface';import{ResendProvider}from'./resend.provider';import{EmailProvider,EmailSendResult}from'./email-provider.interface';
...
@Injectable()export class ProviderGateway{
 ...
 async store(key:string,mime:string,expires:number,signature:string,buffer:Buffer){this.verify('upload',key,mime,expires,signature);const scan=await this.scanBuffer(key,mime,buffer);if(!scan.clean)throw new BadRequestException('File failed malware scan');const stored=await this.compress(buffer,mime);const sha256=createHash('sha256').update(stored).digest('hex');const bucketName=this.config.get<string>('GCS_BUCKET_NAME');if(bucketName){await this.storage.bucket(bucketName).file(key).save(stored,{contentType:mime,resumable:false});await this.event('gcs','upload','SUCCESS',{key,size:stored.length,sha256});return{key,size:stored.length,mimeType:mime,sha256}}const path=this.path(key);await mkdir(dirname(path),{recursive:true});await writeFile(path,stored,{flag:'wx'}).catch(async error=>{if((error as NodeJS.ErrnoException).code==='EEXIST')await writeFile(path,stored);else throw error});await this.event('local-storage','upload','SUCCESS',{key,size:stored.length,sha256});return{key,size:stored.length,mimeType:mime,sha256}}
 ...
 async load(key:string,expires:number,signature:string){this.verify('download',key,'',expires,signature);const bucketName=this.config.get<string>('GCS_BUCKET_NAME');if(bucketName){try{const[data]=await this.storage.bucket(bucketName).file(key).download();await this.event('gcs','download','SUCCESS',{key,size:data.length});return data}catch(error){if((error as{code?:number}).code===404)throw new NotFoundException('File not found');throw error}}try{const data=await readFile(this.path(key));await this.event('local-storage','download','SUCCESS',{key,size:data.length});return data}catch(error){if((error as NodeJS.ErrnoException).code==='ENOENT')throw new NotFoundException('File not found');throw error}}
